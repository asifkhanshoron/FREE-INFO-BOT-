    ...
    async def player_info(self, ctx: commands.Context, uid: str):
        ...
        try:
            ...
            if clan_info:
                ...
                "**┌  DEVELOPER DETAILS**",
                f"**├─ Name : ASIF KHAN **",
                f"**├─ Portfolio : [ASIF KHAN](https://asifkhan-zeta.vercel.app/)**",
                f"**├─ YouTube : [▶ YouTube](https://www.youtube.com/@AsifKhanCheat)**",
                f"**└─ Discord : https://discord.com/invite/s364QDhFGr**"
            ]), inline=False)
                    

            embed.set_footer(text="JOIN OUR SERVER FOR FREE LIKE DAILY ")
            await ctx.send(embed=embed)
